...
use winapi::{
    ...
    um::{
        ...
        winuser::{
            ...
            GWLP_USERDATA, HRAWINPUT, LPMSG, MOUSE_MOVE_RELATIVE, MSG, RAWINPUT, RAWINPUTDEVICE,
            RAWINPUTHEADER, RIDEV_INPUTSINK, RID_INPUT, RIM_TYPEMOUSE, WM_INPUT,
            WM_QUERYENDSESSION, WNDCLASSEXW, RI_MOUSE_WHEEL, ShowCursor, RIDEV_NOLEGACY,RIDEV_CAPTUREMOUSE,ShowWindow, WM_KEYUP, WM_KEYDOWN
        },
...
pub unsafe fn proc_raw_input(hWnd: HWND, l_param: LPARAM) -> bool {
    let mut pcb_size = 0;
    let tx = std::ptr::read(GetWindowLongPtrW(hWnd, GWLP_USERDATA) as *const Arc<window::WindowData>).clone();
    ...
    if get_raw_input_data(ptr::null_mut(), &mut pcb_size) == 0 {
        ...
        if get_raw_input_data(data as LPVOID, &mut pcb_size) == pcb_size {
            ...
            if pressed != xim::ButtonEvent::NOOP {
                tx.xim_tx.send(xim::XIMEvent::KeyboardButton(u16::try_from(key).unwrap(), pressed));
            }
            
            let mouse = ri.data.mouse();    

            tx.xim_tx.send(xim::XIMEvent::MouseButtonEvent(mouse.usButtonFlags));
            if is_mouse_move_relative(ri) {
                ...
                tx.xim_tx.send(xim::XIMEvent::MouseMoveEvent{x:x, y:y, wheel: wheel});
                res = true;
            }
